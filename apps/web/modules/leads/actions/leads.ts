...
export async function getCampaignMetrics(campaignId: string) {
    try {
        ...
        const metrics = {
            ...
            convertedLeads: campaign.leads.filter((l: any) => l.status === 'CONVERTED').length,
            avgLeadScore: campaign.leads.length > 0
                ? Math.round(campaign.leads.reduce((sum: number, l: any) => sum + l.score, 0) / campaign.leads.length)
                : 0,
