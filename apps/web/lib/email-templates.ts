...
export const getNotificationHtml = ({ title, message, link }: { title: string; message: string; link?: string }) => `
<!DOCTYPE html>
<html>
<head>
    <meta name="viewport" content="width=device-width, initial-scale=1.0"/>
    <meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />
    <style>
        body { background-color: #f4f4f5; font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; -webkit-font-smoothing: antialiased; }
        .container { max-width: 600px; margin: 0 auto; padding: 40px 20px; }
        .card { background-color: #ffffff; border: 1px solid #e4e4e7; border-radius: 12px; padding: 32px; }
        .heading { color: #18181b; font-size: 20px; font-weight: bold; margin: 0 0 12px; }
        .text { color: #71717a; font-size: 15px; line-height: 24px; margin: 0 0 20px; }
        .btn { display: inline-block; background-color: #0d9488; color: #ffffff; font-weight: bold; text-decoration: none; padding: 12px 24px; border-radius: 8px; font-size: 14px; }
        .footer { margin-top: 32px; text-align: center; color: #a1a1aa; font-size: 12px; }
    </style>
</head>
<body>
    <div class="container">
        <div class="card">
            ...
            ${link ? `<a href="${link}" class="btn">Ver Detalles</a>` : ''}
        </div>
        <div class="footer">
            &copy; 2026 LegacyMark &middot; Notificación automática del sistema.
        </div>
    </div>
</body>
</html>
