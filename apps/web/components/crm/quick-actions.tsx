...
function ActionBtn({ label, code, className }: { label: string; code: string; className?: string }) {
    return (
        <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 font-mono text-xs font-bold uppercase tracking-widest rounded-sm transition-all duration-200 cursor-pointer ${className}`}>
    ...
    return (
        <div className="flex flex-wrap items-center gap-2 mt-3">
            ...
            <CreateDealDialog
                companyId={companyId}
                trigger={
                    <button className="flex items-center gap-1.5 px-4 py-2 font-mono text-xs font-bold uppercase tracking-widest text-white rounded-sm transition-all hover:shadow-[0_0_20px_-8px_rgba(13,148,136,0.6)] hover:-translate-y-0.5"
            ...
            <div className="flex items-center gap-0"
                ...
                <CreateLeadDialog
                    companyId={companyId}
                    trigger={
                        <button className="flex items-center gap-1.5 px-3 py-2 font-mono text-xs font-bold uppercase tracking-widest text-slate-400 hover:text-teal-400 hover:bg-teal-950/30 transition-all">
                ...
                <QuickNoteDialog
                    companyId={companyId}
                    trigger={
                        <button className="flex items-center gap-1.5 px-3 py-2 font-mono text-xs font-bold uppercase tracking-widest text-slate-400 hover:text-amber-400 hover:bg-amber-950/20 transition-all">
